...
class AdminPermissionRegistrar
{
    ...
    /**
     * AdminPermissionRegistrar constructor.
     */
    public function __construct(CacheManager $cacheManager)
    {
        $this->permissionClass = config('admin-permission.models.permission');
        $this->roleClass = config('admin-permission.models.role');
    ...
    {
        $this->cacheExpirationTime = config('admin-permission.cache.expiration_time') ?: \DateInterval::createFromDateString('24 hours');

        $this->cacheKey = config('admin-permission.cache.key');

        $this->pivotRole = config('admin-permission.column_names.role_pivot_key') ?: 'admin_role_id';
        $this->pivotPermission = config('admin-permission.column_names.permission_pivot_key') ?: 'admin_permission_id';
    ...
    {
        // the 'default' fallback here is from the admin-permission.php config file,
        // where 'default' means to use config(cache.default)
        $cacheDriver = config('admin-permission.cache.store', 'default');
    ...
    /**
     * Register the admin permission check method on the gate.
    ...
    public function registerPermissions(Gate $gate): bool
    {
        $gate->before(function (Authorizable $user, string $ability, array &$args = []) {
            ...
            if (method_exists($user, 'checkAdminPermissionTo')) {
                return $user->checkAdminPermissionTo($ability, $guard ?? null) ?: null;
            }
        });
    ...
    /**
     * Clear already-loaded admin permissions collection.
    ...
    public function setPermissionClass($permissionClass)
    {
        ...
        config()->set('admin-permission.models.permission', $permissionClass);
    ...
    public function setRoleClass($roleClass)
    {
        ...
        config()->set('admin-permission.models.role', $roleClass);
    ...
    {
        return $this->permissionClass::select()->with('adminRoles')->get();
    }
    ...
    {
        $this->except = config('admin-permission.cache.column_names_except', ['created_at', 'updated_at', 'deleted_at']);

        $adminPermissions = $this->getPermissionsWithRoles()
        ...
        $adminRoles = array_values($this->cachedRoles);
        $this->cachedRoles = [];

        return ['alias' => array_flip($this->alias)] + compact('adminPermissions', 'adminRoles');
    }

    private function getSerializedRoleRelation($permission): array
    {
        if (! $permission->adminRoles->count()) {
            return [];
        }

        if (! isset($this->alias['roles'])) {
            $this->alias['adminRoles'] = 'r';
            $this->aliasModelFields($permission->adminRoles[0]);
        }

        return [
            'r' => $permission->adminRoles->map(function ($role) {
    ...
    private function getHydratedPermissionCollection(): Collection
    {
        ...
        return Collection::make(array_map(
            fn ($item) => $permissionInstance->newInstance([], true)
                ...
                ->setRelation('adminRoles', $this->getHydratedRoleCollection($item['r'] ?? [])),
